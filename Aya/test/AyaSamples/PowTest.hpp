...
inline void GenerateMRsForPowLowPrecision(std::function<double(double, double)> testedFunction, const std::string &outputMRFile,
...
inline void GenerateMRsForPowHighPrecision(std::function<double(double, double)> testedFunction, const std::string &outputMRFile,
        size_t inputTransformerChainLength,
        size_t outputTransformerChainLength) {
    // Prepare an array of transformer functions
    const std::vector<std::function<void(double &, double)> > inputTransformerFuncs = {Div, Mul, Add, Sub, Noop};
    ...
    std::vector<std::vector<std::any> > testedInputs;
    std::vector<Aya::MetamorphicRelation> finalMRs;
    testedInputs.push_back({Aya::GenerateDoublesAsAny(3, 0, 5)});
    testedInputs.push_back({Aya::GenerateDoublesAsAny(3, 0, 3)});
    mrBuilder.SearchForMRs(testedInputs, inputTransformerChainLength, outputTransformerChainLength, overallMatchCount, finalMRs);

    std::vector<std::vector<std::any> > validatorInputs;
    validatorInputs.reserve(2);
    validatorInputs.push_back({Aya::GenerateDoublesAsAny(7, 0, 5)});
    validatorInputs.push_back({Aya::GenerateDoublesAsAny(7, 0, 2)});

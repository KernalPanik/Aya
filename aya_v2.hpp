...
// ===========================================================================
// ForkCombineTransformer: fork input, apply two sub-transformers, combine
// ===========================================================================
// Snapshots the input, applies left_ and right_ to separate copies,
// then combines results via combiner_. Both sub-transformers see the
// original input, solving the sequential-mutation problem.
...
public:
    ...
    ForkCombineTransformer(
        ...
        , combiner_name_(std::move(combiner_name))
    {}

    void Apply(std::any& data) override {
        std::any left_copy = data, right_copy = data;
        left_->Apply(left_copy);
        right_->Apply(right_copy);
        data = combiner_(std::any_cast<T>(left_copy), std::any_cast<T>(right_copy));
    }

    std::string ToString(const char* target, size_t idx) const override {
        std::string left_str = left_->ToString(target, idx);
        std::string right_str = right_->ToString(target, idx);
        return combiner_name_ + "( " + left_str + ", " + right_str + " )";
    }
    ...
    std::shared_ptr<ITransformer> Clone() const override {
        return std::make_shared<ForkCombineTransformer<T>>(
            left_->Clone(), right_->Clone(), combiner_, combiner_name_
        );
    }

private:
    TransformerPtr left_;
    TransformerPtr right_;
    Combiner combiner_;
    std::string combiner_name_;
};

/// Create a single fork-combine transformer.
template <typename T>
TransformerPtr MakeForkCombine(
    ...
    return std::make_shared<ForkCombineTransformer<T>>(
        std::move(left), std::move(right), std::move(combiner), std::move(combiner_name)
    );
}

/// Batch-create fork-combine transformers from all pairwise combinations
/// of primitives and combiners. Returns O(N^2 * C) transformers.
/// Users explicitly call this and add results to their pools.
template <typename T>
std::vector<TransformerPtr> MakeForkCombinePool(
    ...
    out.reserve(primitives.size() * primitives.size() * combiners.size());
    for (size_t i = 0; i < primitives.size(); ++i) {
        for (size_t j = 0; j < primitives.size(); ++j) {
            if (i == j) continue;
            for (const auto& [name, fn] : combiners) {
                out.push_back(std::make_shared<ForkCombineTransformer<T>>(
                    primitives[i]->Clone(), primitives[j]->Clone(), fn, name
                ));
            }
        }
    }

...
// ===========================================================================
// 14. NEW: ForkCombineTransformer tests
// ===========================================================================

void TestForkCombineManual() {
    SECTION("15. ForkCombineTransformer — manual construction");

    // Test 1: sin/cos fork + divide = tan
    TEST("ForkCombine(Sin, Cos, Div) computes tan(x)");
    {
        auto sin_tf = Aya::MakeTransformer<double>("Sin", std::function<void(double&)>(
            [](double& x) { x = std::sin(x * PI / 180.0); }));
        auto cos_tf = Aya::MakeTransformer<double>("Cos", std::function<void(double&)>(
            [](double& x) { x = std::cos(x * PI / 180.0); }));

        std::function<double(double, double)> div_fn = [](double a, double b) -> double {
            return a / b;
        };
        ...
        for (double deg : {30.0, 45.0, 60.0, 75.0}) {
            std::any val = deg;
            tan_fc->Apply(val);
            double result = std::any_cast<double>(val);
            double expected = std::tan(deg * PI / 180.0);
            ASSERT_TRUE(ApproxEq(result, expected),
                "tan(" + std::to_string(deg) + "): got " + std::to_string(result)
                + ", expected " + std::to_string(expected));
        }
        PASS();
    }

    // Test 2: ToString produces readable output
    TEST("ForkCombine ToString");
    {
        auto sin_tf = Aya::MakeTransformer<double>("Sin", std::function<void(double&)>(
            [](double& x) { x = std::sin(x); }));
        auto cos_tf = Aya::MakeTransformer<double>("Cos", std::function<void(double&)>(
            [](double& x) { x = std::cos(x); }));

        auto fc = Aya::MakeForkCombine<double>(sin_tf, cos_tf,
            std::function<double(double,double)>([](double a, double b) { return a / b; }),
            "Div");

        std::string s = fc->ToString("input", 0);
        ASSERT_TRUE(s.find("Div") != std::string::npos, "ToString missing 'Div'");
        ASSERT_TRUE(s.find("Sin") != std::string::npos, "ToString missing 'Sin'");
        ASSERT_TRUE(s.find("Cos") != std::string::npos, "ToString missing 'Cos'");
        ASSERT_TRUE(s.find("input[0]") != std::string::npos, "ToString missing 'input[0]'");
        std::cout << "    ToString: " << s << "\n";
        PASS();
    }

    // Test 3: Clone produces independent copy
    TEST("ForkCombine Clone independence");
    {
        auto left = Aya::MakeTransformer<double>("Double", std::function<void(double&)>(
            [](double& x) { x *= 2; }));
        auto right = Aya::MakeTransformer<double>("Triple", std::function<void(double&)>(
            [](double& x) { x *= 3; }));

        auto fc = Aya::MakeForkCombine<double>(left, right,
            std::function<double(double,double)>([](double a, double b) { return a + b; }),
            "Add");

        auto cloned = fc->Clone();

        std::any v1 = 5.0, v2 = 5.0;
        fc->Apply(v1);
        cloned->Apply(v2);
        ASSERT_TRUE(ApproxEq(std::any_cast<double>(v1), std::any_cast<double>(v2)),
            "Clone should produce same result");
        // 5*2 + 5*3 = 10 + 15 = 25
        ASSERT_TRUE(ApproxEq(std::any_cast<double>(v1), 25.0),
            "Expected 25, got " + std::to_string(std::any_cast<double>(v1)));
        PASS();
    }
}

void TestForkCombinePool() {
    SECTION("16. MakeForkCombinePool — batch generation");

    TEST("pool size matches N*(N-1)*C");
    {
        ...
        auto pool = Aya::MakeForkCombinePool<double>(primitives, combiners);
        // 3 primitives * 2 partners * 4 combiners = 24
        size_t expected = 3 * 2 * 4;
        ASSERT_TRUE(pool.size() == expected,
            "Expected " + std::to_string(expected) + " fork-combine transformers, got "
            + std::to_string(pool.size()));
        PASS();
    }
}

void TestForkCombineEngineDiscovery() {
    SECTION("17. ForkCombine in MREngine — compound MR discovery");

    TEST("engine discovers compound MRs with fork-combine input transformers");
    {
        // f(x) = x (identity). Fork-combine input transformers like
        // Add(Double(x), Triple(x)) = 5x should match output Mul(x, 5).
        auto engine = Aya::MREngine<double, double>(
            [](double x) -> double { return x; },
            DoubleEq
        );

        // Build fork-combine pool from simple primitives as input transforms
        std::vector<Aya::TransformerPtr> primitives = {
            Aya::MakeTransformer<double>("Double", std::function<void(double&)>(
                [](double& x) { x *= 2; })),
            Aya::MakeTransformer<double>("Triple", std::function<void(double&)>(
                [](double& x) { x *= 3; })),
            Aya::MakeTransformer<double>("Negate", std::function<void(double&)>(
                [](double& x) { x = -x; })),
        };
        ...
        engine.AddInputTransformers(0, fc_pool);

        // Output transformers: multipliers that should match fork-combine results
        engine.AddOutputTransformers({
            Aya::MakeTransformer<double, double>("Mul", std::function<void(double&,double)>(
                [](double& x, double c) { x *= c; }), 5.0),   // 2x+3x
            Aya::MakeTransformer<double, double>("Mul", std::function<void(double&,double)>(
                [](double& x, double c) { x *= c; }), -1.0),  // 2x-3x = -x
            Aya::MakeTransformer<double, double>("Mul", std::function<void(double&,double)>(
                [](double& x, double c) { x *= c; }), 6.0),   // 2x*3x = 6x² won't match
            Aya::MakeTransformer<double>("Negate", std::function<void(double&)>(
                [](double& x) { x = -x; })),
        });

        auto sample = {GenDoubles(4, 1, 10)};
        auto verify = {GenDoubles(4, 1, 10)};

        auto mrs = engine.Search(sample, verify, 1, 1, 0, 0, 1.0f);
        auto consolidated = Aya::ConsolidateMRs(mrs);

        ASSERT_TRUE(!consolidated.empty(),
            "engine: expected MRs using fork-combine input transformers");

        // Print discovered MRs
        for (const auto& [text, stats] : consolidated) {
            std::cout << "    " << text << "\n";
        }
        PASS();
...
void TestForkCombineBenchmark() {
    SECTION("18. ForkCombine Benchmark — timing comparison");
    TEST("fork-combine pool timing");
    {
        auto func = [](double x) -> double { return std::sin(x * PI / 180.0); };

        // Without fork-combine
        auto start1 = std::chrono::high_resolution_clock::now();
        {
            auto engine = Aya::MREngine<double, double>(func, DoubleEq);
            engine.AddInputTransformers(0, MakeDoubleNoArgTransformers());
            engine.AddOutputTransformers(MakeDoubleNoArgTransformers());

            auto sample = {GenDoubles(3, 30, 90)};
            auto verify = {GenDoubles(3, 30, 90)};
            auto mrs = engine.Search(sample, verify, 1, 1, 0, 0, 1.0f);
        }
        auto end1 = std::chrono::high_resolution_clock::now();
        auto ms1 = std::chrono::duration_cast<std::chrono::milliseconds>(end1 - start1).count();

        // With fork-combine pool added
        auto start2 = std::chrono::high_resolution_clock::now();
        {
            auto engine = Aya::MREngine<double, double>(func, DoubleEq);
            engine.AddInputTransformers(0, MakeDoubleNoArgTransformers());

            // Build fork-combine pool from a subset of primitives
            std::vector<Aya::TransformerPtr> primitives = {
                Aya::MakeTransformer<double>("Sin", std::function<void(double&)>(
                    [](double& x) { x = std::sin(x * PI / 180.0); })),
                Aya::MakeTransformer<double>("Cos", std::function<void(double&)>(
            ...
            auto fc_pool = Aya::MakeForkCombinePool<double>(primitives, combiners);
            auto base_pool = MakeDoubleNoArgTransformers();
            base_pool.insert(base_pool.end(), fc_pool.begin(), fc_pool.end());
            engine.AddOutputTransformers(base_pool);
        ...
        std::cout << "    Without fork-combine: " << ms1 << " ms\n";
        std::cout << "    With fork-combine:    " << ms2 << " ms\n";
        std::cout << "    Ratio: " << (ms1 > 0 ? static_cast<double>(ms2) / ms1 : 0) << "x\n";
        PASS();
    }
}
...
int main() {
    ...
    TestResultAsSource();
    TestForkCombineManual();
    TestForkCombinePool();
    TestForkCombineEngineDiscovery();
    TestForkCombineBenchmark();

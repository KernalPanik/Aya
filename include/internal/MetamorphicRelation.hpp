...
namespace Aya {
    struct MetamorphicRelation {
        using TransformerPair = std::pair<size_t, std::shared_ptr<ITransformer>>;
        std::vector<std::shared_ptr<TransformerPair>> InputTransformers;
        std::vector<std::shared_ptr<TransformerPair>> OutputTransformers;
    ...
    inline void DumpMRsToFile(const std::vector<MetamorphicRelation> &MRs, const std::string &path,
        ...
        for (const auto &mr: consolidatedMrs) {
            if (mr.second.second >= successRateThreshold)
            {
    ...
    inline void ProduceMREvaluationReport(const std::vector<MetamorphicRelation> &MRs,
                                          ...
                                          const float minSuccessRate = 1.0f) {
        ...
        size_t validationInputCount = 1;
        for (auto &i : usedValidationInputs) {
            validationInputCount *= i.size();
        }
        outputFile << validationInputCount << " Inputs Used For Validation: " << std::endl;
        ...
        outputFile.close();
        DumpMRsToFile(MRs, MRFilePath, minSuccessRate, std::ios_base::app);
    ...
    // MR Score is the success rate of MR test for a given input.
    template<typename T, typename U, typename... Args>
    void CalculateMRScore(std::function<T(Args...)> func, std::function<bool(U, U)> comparerFunction,
                          ...
                          const size_t leftValueIndex, const size_t rightValueIndex, bool overrideArgs = false, bool logStates = false) {
        ...
        for (auto &MR: MRs) {
            ...
            while (!inputIterator.isDone()) {
                ...
                for (auto &p: pos) {
                    ...
                    try {
                        // ValidatorCycleCount is needed for debugging purposes only. I don't have a better solution to quickly check a specific combination
                        validatorCycleCount++;
                        ...
                        if (Aya::ValidateInputVariant<T, U, Args...>(

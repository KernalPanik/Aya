...
namespace Aya {
    ...
    std::vector<std::vector<std::tuple<Arg>>> MakeArgPool(size_t count, const std::vector<Arg>& values, bool addNoop = false) {
        std::vector<std::vector<std::tuple<Arg>>> pool;
        for (size_t i = 0; i < count; ++i) {
            std::vector<std::tuple<Arg>> args;
        ...
        if (addNoop) {
            pool.push_back({std::tuple<Arg>{0}});
        }
        return pool;
    }

    template<typename T, typename Arg, typename Func>
    std::vector<std::shared_ptr<Aya::ITransformer>> PrepareTransformers(
        const std::vector<Func>& funcs,
        const std::vector<std::string>& names,
        const std::vector<std::vector<std::tuple<Arg>>>& argPool)
    {
        return Aya::TransformBuilder<T, Arg>().GetTransformers(funcs, names, argPool);
    }
}

...
inline bool compareArrays(std::array<double, 2> x, std::array<double, 2> y) {
    return fabs(x[0] - y[0]) < 1e-6 && fabs(x[1] - y[1]) < 1e-6;
}

inline std::string arrayToString(const std::any& v) {
    std::stringstream ss;
